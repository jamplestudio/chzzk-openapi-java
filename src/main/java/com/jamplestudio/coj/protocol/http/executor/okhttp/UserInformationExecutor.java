...
public class UserInformationExecutor implements HttpRequestExecutor<UserInformationRequest, UserInformationResponse, OkHttpClient> {
    ...
    private static final @NotNull String URL = "https://openapi.chzzk.naver.com/open/v1/users/me";
    ...
    public @NotNull Optional<UserInformationResponse> execute(
        ...
        try (Response response = client.getNativeHttpClient().newCall(request).execute()) {
            if (response.isSuccessful() && response.body() != null) {
                UserInformationResponse responseInst = GSON.fromJson(response.body().string(), UserInformationResponse.class);
